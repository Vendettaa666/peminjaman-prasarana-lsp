...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('user.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.4 rounded-lg shadow-md transition duration-200 flex items-center gap-2">
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-4 text-sm font-semibold text-gray-600 uppercase">Informasi User</th>
                    <th class="px-6 py-4 text-sm font-semibold text-gray-600 uppercase">Role</th>
                    <th class="px-6 py-4 text-sm font-semibold text-gray-600 uppercase">NISN / Kelas</th>
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex justify-center items-center gap-3">
                            <a href="{{ route('user.show', $data->id) }}" class="text-blue-600 hover:text-blue-900 font-medium text-sm">Lihat</a>
                            <a href="{{ route('user.edit', $data->id) }}" class="text-yellow-600 hover:text-yellow-900 font-medium text-sm">Edit</a>

                            <form action="{{ route('user.destroy', $data->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus user ini?')">
                                @csrf
                                @method('DELETE')
    ...
    @if($alldata->isEmpty())
        <div class="text-center py-10">
            <p class="text-gray-500 italic">Belum ada data user.</p>
        </div>
    @endif
</div>
